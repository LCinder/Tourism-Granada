			...
			case "comprarEntradas":
				try {
					...
					if(monumento == "Alhambra") {
						...
						return res.send({fulfillmentText: "El precio para "
						+ tipoTicket + " es de " + generarEntero(0, 20) + "€. ¿Deseas comprarla?"});
					}
...
/******************************************************************************/
/******************************************************************************/
			case "respuestaAfirmativaParadaComer":
				try {
					let bar = elegirNSitios("bares", 1)
					let cad = "En ese caso, te recomiendo que vayas a " + bar[0]
					return res.send({fulfillmentText: cad})
				}
				catch (err) {
					return res.send({fulfillmentText: "Error: " + err})
